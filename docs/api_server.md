...
| **Discussion Router** | `/discussion/chat` | Authenticated, synchronous, ephemeral article-grounded chat. It semantically retrieves ingested articles from a selected rolling timeframe and uses the configured LLM without persisting prompts or answers. |
...
| **Settings Router** | `/settings` | Read/update the alert policy (`/settings/alert-policy`) read configuration/scoring presets (`/settings/presets`), and GET/PUT shared watched topics (`/settings/watched-topics`, `{topics: string[]}`; up to 30 unique trimmed phrases of 1–300 characters) ([settings.py](../market-watch-bot/api_server/app/api/routers/settings.py)). |
